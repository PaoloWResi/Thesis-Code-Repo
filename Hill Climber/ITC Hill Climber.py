...
while runCount <= 21:
    ...
    def swap_random_lecture(timetable):
        while True:
            ...
            lecture = timetable[day][period][room]

            if lecture != 0:
                if isinstance(lecture, list):
                    selected_lecture = lecture.pop(random.randint(0, len(lecture) - 1))
                    if not lecture:
                        timetable[day][period][room] = 0
                else:
        ...
        while True:
            ...
            destination = timetable[new_day][new_period][new_room]

            if destination == 0:
                timetable[new_day][new_period][new_room] = selected_lecture
                break
            elif isinstance(destination, list):
                destination.append(selected_lecture)
                break
            else:
                timetable[new_day][new_period][new_room] = [destination, selected_lecture]
                break


    current_hard, current_soft = evaluate_timetable(timetable)

    for _ in range(10000):
        if current_hard == 0 and current_soft == 0:
            break

        temp_timetable = copy.deepcopy(timetable)
        swap_random_lecture(temp_timetable)
        new_hard, new_soft = evaluate_timetable(temp_timetable)

        if new_hard < current_hard or (new_hard == current_hard and new_soft < current_soft):
            hard_violations_list.append(new_hard)
            soft_violations_list.append(new_soft)
            timetable = temp_timetable
            current_hard, current_soft = new_hard, new_soft
        elif new_hard > current_hard or (new_hard == current_hard and new_soft > current_soft) or (new_hard == current_hard and new_soft == current_soft):
            hard_violations_list.append(current_hard)
            soft_violations_list.append(current_soft)
